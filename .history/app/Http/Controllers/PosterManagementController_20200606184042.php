...
class PosterManagementController extends Controller
{
    ...
    private function checkPosterExist($id){
        if($id !== '')
        {
            $posters = $posters = DB::table('categories')->join('posters', 'posters.id_category', '=', 'category.id')->join('users', 'users.id', '=', 'posters.id_creator')->where('users.id', '=', Auth::id())->where('posters.id', '=', $id)->get(['category.title as categorytitle', 'posters.id as poster_id', 'category.id as category_id', 'users.*', 'posters.*']);
            if(count($posters))
            {
